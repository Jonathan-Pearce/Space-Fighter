...
public class GamePanel extends JPanel implements MouseMotionListener, MouseListener, KeyListener {
	...
	// declares ship and user and allows ship and user class to be accessed by
	// GamePanel
	Ship ship = new Ship();
	User user = new User();
	...
	// all images are drawn to offScreenG but not displayed, then offScreenG is
	// displayed itself
	...
	GamePanel() throws IOException, URISyntaxException { // will stop/crash program in case background example cannot be found
		...
		// Creates Bullet for every bullet in the array and allows user's
		// bullets to access Bullet class
		for (int i = 0; i < 30; i++) {
	...
	public void paintComponent(Graphics g) {
		...
		if (user.mainMenu) {
			...
			// Draw 2 backgrounds side by side in order to allow for proper
			// scrolling
			...
			// if enemy has been killed but enemy still has active bullets,
			// bullets will be drawn
			for (int i = 0; i < Enemy.size(); i++) {
				if (Enemy.get(i).shipActive) {
					offScreenG.drawImage(Enemy.get(i).img, Enemy.get(i).enemyX - Enemy.get(i).img.getWidth() / 2,
							Enemy.get(i).enemyY - Enemy.get(i).img.getHeight() / 2, this);
					for (int j = 0; j < 20; j++) {
	...
	public void run() throws InterruptedException, IOException {
		...
		while (true) {
			...
			if (user.playing) {
				...
				// Checking what direction ship is moving and them moves ship in
				// that direction
				if (ship.down) {
					ship.moveDown();
				}
				...
				if (counter % ((int) 50 - (user.level * 2)) == 0) {
					...
					// randomly assigns one of the differnt types of enemies to
					// new enemy
				...
				// cycles through active enemies and determines whether it is
				// time to fire enemyBullet
				for (int i = 0; i < Enemy.size(); i++) {
				...
				// Score is incremented in different amounts depending on what
				// enemy is hit
					...
					if (Enemy.get(i).active) {
						for (int j = 0; j < 30; j++) {
							if (myBullets[j].active) {
								if (Math.sqrt((Math.pow(myBullets[j].bulletX - (Enemy.get(i).enemyX), 2)
						...
						// If collision, enemy is removed and user's health goes
						// done
						if (Math.sqrt(
								(Math.pow((ship.shipX) - (Enemy.get(i).enemyX), 2) + (Math.pow((ship.shipY) - (Enemy.get(i).enemyY), 2)))) < 44) {
							user.health -= 10;
							Enemy.get(i).shipDeactivate();
						}
					}
				...
				// If bullet and ship collide bullet is removed and ship's
				// health decreases
					...
					if (Enemy.get(j).active) {
						for (int k = 0; k < 20; k++) {
							if (Enemy.get(j).bulletActive(k)) {
								if ((Math.sqrt(
										(Math.pow((ship.shipX) - Enemy.get(j).getX(k), 2)) + Math.pow(ship.shipY - Enemy.get(j).getY(k), 2))) < 40) {
									user.health -= 5;
				...
				// Randomly determines whether enemy firetimer should be
				// incremented
				...
				// checks whether user's health is greater than zero to
				...
				// depending on which type of enemy is which bullets are
				// animated in respective manner
				...
				// Cycles through enemies and removes any inactive enemies from
				// the array
				for (int i = 0; i < Enemy.size(); i++) {
					if (!(Enemy.get(i).active)) {
						Enemy.remove(i);
				...
				// checks whether enemies who have been hit still have any
				// active on screen bullets
				for (int i = 0; i < Enemy.size(); i++) {
					if (!(Enemy.get(i).shipActive)) {
	...
	public void mouseReleased(MouseEvent e) {
		// Checks whether user has pressed start game button and sets game
		// playing to true
		if (user.mainMenu) {
	...
	public void keyPressed(KeyEvent e) {
		switch (e.getKeyCode()) {
		// If directional key is pressed, respective direction boolean is set to
		// true
		case 87:
			ship.up = true;
			break;
	...
	public void keyReleased(KeyEvent e) {
		switch (e.getKeyCode()) {
		// If directional key is released, respective direction boolean is set
		// to false
		case 87:
			ship.up = false;
			break;
